...
import { Card, Col, DatePicker, Form, Input, InputNumber, Modal, Row, Select } from 'hzero-ui';
      ...
      >
        <Row gutter={24}>
          ...
          {getFieldValue('cycleFlag') && (
            <Col span={12}>
              <Form.Item
                label={intl.get('hsdr.concRequest.model.concRequest.startDate').d('周期开始时间')}
                {...formLayout}
              >
          ...
          {getFieldValue('cycleFlag') && (
            <Col span={12}>
              <Form.Item
                label={intl.get('hsdr.concRequest.model.concRequest.endDate').d('周期结束时间')}
                {...formLayout}
              >
