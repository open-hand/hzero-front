...
export default class ValueList extends React.Component {
  ...
  componentDidUpdate() {
    ...
    const { shouldUpdateProps: scu, needLazyLoad = true } = this.state;
    const {
      lovCode,
      queryUrl,
      queryService,
    ...
    if (lazyLoad && !needLazyLoad && !isEqual(scu, shouldUpdateProps)) {
      // warn 如果更新了 更新条件 并且是懒加载, 则将需要懒加载设置为空
      // eslint-disable-next-line react/no-did-update-set-state
      this.setState({ needLazyLoad: true });
    } else if (!lazyLoad && !isEqual(scu, shouldUpdateProps)) {
      this.handleDeal$Options();
    }
  }
