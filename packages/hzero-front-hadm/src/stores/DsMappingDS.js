...
const tableDs = {
  ...
  transport: {
    read: () => {
      const url = isTenantRoleLevel() ? `${HZERO_ADM}/v1/${organizationId}/sp-service-datasource` : `${HZERO_ADM}/v1/sp-service-datasource`;
      return {
        url,
        method: 'GET',
      };
    ...
    destroy: ({ data }) => {
      ...
      return {
        data: other,
        url: isTenantRoleLevel() ? `${HZERO_ADM}/v1/${organizationId}/sp-service-datasource` : `${HZERO_ADM}/v1/sp-service-datasource`,
        method: 'DELETE',
      };
    },
  },
...
const detailDs = {
  ...
  fields: [
...

    {
      ...
      type: 'string',
      label: intl.get('hadm.dsMapping.model.dsMapping.connectionTimeoutMilliseconds').d('连接超时毫秒'),
    },
    {
  ...
  transport: {
    ...
    read: ({ dataSet }) => {
      const { queryParameter: { serviceDatasourceId } } = dataSet;
      const url = isTenantRoleLevel() ? `${HZERO_ADM}/v1/${organizationId}/sp-service-datasource/${serviceDatasourceId}` : `${HZERO_ADM}/v1/sp-service-datasource/${serviceDatasourceId}`;
      return {
        url,
        method: 'GET',
        params: '',
